...
description: 优先通过 Antigravity 内置浏览器代理（或 Playwright-CLI）连接真实浏览器，跟踪 sign/enc/token 的生成链路，识别运行时入口函数与依赖，然后生成 JSRPC 注入代码、Flask 代理代码以及带校验输出的 Burp autoDecoder 对接文档。本技能只生成代码，不对未授权目标测试。
---
...
- 任务必须保持主链路完整：浏览器连接、参数链路跟踪、入口发现、JSRPC 生成、Flask 代理生成，以及 Burp autoDecoder 对接。

## 工具偏好
- **默认工具**：Antigravity 内置的 `browser_subagent`。
- **通用/备选工具**：在非 Antigravity 环境或需要更细粒度脚本控制时，使用 `playwright-cli` 技能及相关指令。

## 工作边界
- 使用 `browser_subagent` 或 `playwright-cli` 指令完成调查，包括：
  - 页面导航 (`goto`)、刷新、等待、截图与快照。
  - 网络请求列表与详情读取 (`playwright-cli network`)。
  - 注入观测代码 (`browser_subagent` 交互或 `playwright-cli run-code`)。
  - 控制台日志读取 (`playwright-cli console`)。
- 调查重点放在：
  - 目标请求的网络捕获与归因。
  - 从请求反推源码位置与参数入口。
...
- Phase 1. 浏览器连接与请求复现
  - 通过内置浏览器代理或 `playwright-cli` 连接真实浏览器，打开目标页面，并复现通向目标参数的请求链路。
...
## 参考文件装载规则
- 需要确认浏览器能力边界时，先读取 `references/browser-capability-matrix.md`。
