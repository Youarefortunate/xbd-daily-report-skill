...
async def polish_report(commits, fake_items, feishu_extra, prompt_path):
    """步骤 2: AI 润色处理"""
...
async def run_daily_bot():
    ...
    if weekdays_config:
        ...
        if current_day not in allowed_days:
            log.info(f"📅 [跳过] 今日 (周{current_day}) 不在预设工作日 [{weekdays_config}] 内，程序已停止。")
            return
    ...
    # 2. AI 润色
    report_items = await polish_report(commits, fake_items, feishu_extra, prompt_path)
